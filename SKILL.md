...
Assume the user's app repository is the current workspace. AgentDemo is a dependency in that repo, not the product being built.
...
3. Preflight AgentDemo availability:
  - Check `package.json` first (`dependencies`, `devDependencies`, or `scripts`) for `agentdemo` usage before installing anything.
  - If `agentdemo` is already present, run via `npx agentdemo ...` from project root.
  - If `agentdemo` is not present, ask the user whether to install it in the current repo or use an existing setup from another repo/path.
  - Only run `npm install agentdemo` in the current repo after explicit user confirmation.
  - If `npx agentdemo` fails due to package/build issues, install a release that ships `dist/`, or follow Maintainers flow when applicable.
...
### Locators and AgentDemo coordinates
